//*****************************************************************************
// Modified for CMSIS by Quantum Leaps on Mar-04-2010
...
//
//*****************************************************************************
#define GPIO_OLEDDC_PIN       (1 << 7)
#define GPIO_OLEDEN_PIN       (1 << 6)
#define GPIO_OLEDFSS_PIN      (1 << 3)
...
//
//*****************************************************************************
static volatile unsigned char g_bSSIEnabled = 0;
...
RITWriteCommand(const unsigned char *pucBuffer, unsigned long ulCount)
{
    ...
    //
    GPIOC->DATA_Bits[GPIO_OLEDDC_PIN] = 0;
    ...
    while(ulCount != 0)
    {
        ...
        SSI0->DR = *pucBuffer++;  // Write the data to the SSI
        ...
        ulTemp = SSI0->DR;        // Read data from SSI

        ulCount--;   // Decrement the BYTE counter.
    }
}
...
RITWriteData(const unsigned char *pucBuffer, unsigned long ulCount)
{
    ...
    //
    GPIOC->DATA_Bits[GPIO_OLEDDC_PIN] = GPIO_OLEDDC_PIN;
...
RIT128x96x4Enable(unsigned long ulFrequency)
{
    ...
    //
    SSI0->CR1 = 0;
    ulTemp = SystemCoreClock / ulFrequency;
    ulPreDiv = 0;
    do {
        ulPreDiv += 2;
        ulSCR = (ulTemp / ulPreDiv) - 1;
    }
    while (ulSCR > 255);
    SSI0->CPSR = ulPreDiv;
    SSI0->CR0  = (ulSCR << 8) | (1 << 6) | (8 - 1);

    //
    // (Re)Enable SSI control of the FSS pin.
    //
    GPIOA->DIR   &= ~GPIO_OLEDFSS_PIN;
    GPIOA->AFSEL |=  GPIO_OLEDFSS_PIN;
    GPIOA->DR8R  |=  GPIO_OLEDFSS_PIN;    // 8mA strength
    GPIOA->ODR   &= ~GPIO_OLEDFSS_PIN;
    GPIOA->PUR   |=  GPIO_OLEDFSS_PIN;
    GPIOA->PDR   &= ~GPIO_OLEDFSS_PIN;
    GPIOA->DEN   |=  GPIO_OLEDFSS_PIN;
...
RIT128x96x4Disable(void)
{
    ...
    GPIOA->PDR   &= ~GPIO_OLEDFSS_PIN;
    GPIOA->DEN   |=  GPIO_OLEDFSS_PIN;
    GPIOA->AMSEL &= ~GPIO_OLEDFSS_PIN;

    GPIOA->DATA_Bits[GPIO_OLEDFSS_PIN] = GPIO_OLEDFSS_PIN;
}
...
RIT128x96x4Init(unsigned long ulFrequency)
{
    ...
    //
    SYSCTL->RCGC1 |= (1 <<  4);                    /* enable clock to SSI0  */
    ...
    GPIOA->DIR   &= ~ulPins;
    GPIOA->AFSEL |=  ulPins;
    GPIOA->DR8R  |=  ulPins;    // 8mA strength
    GPIOA->ODR   &= ~ulPins;
    GPIOA->PUR   |=  ulPins;
    GPIOA->PDR   &= ~ulPins;
    GPIOA->DEN   |=  ulPins;
    ...
    //
    ulPins = GPIO_OLEDDC_PIN | GPIO_OLEDEN_PIN;
    GPIOC->DIR   |=  ulPins;
    GPIOC->AFSEL &= ~ulPins;
    GPIOC->DR8R  |=  ulPins;    // 8mA strength
    GPIOC->ODR   &= ~ulPins;
    GPIOC->PUR   &= ~ulPins;
    GPIOC->PDR   &= ~ulPins;
    GPIOC->DEN   |=  ulPins;
