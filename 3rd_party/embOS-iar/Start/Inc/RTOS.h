...
//
// OS_INT_PreserveAll() / OS_INT_RestoreAll() can be implemented identically for all cores.
//
...
//
 ...
 || (defined (__ARM8M_BASELINE__) && (__CORE__ == __ARM8M_BASELINE__))
  ...
  //
    ...
    #define OS_INT_Disable()    { OS_U32 primask;                            \
  ...
  #define OS_INT_Preserve(p)    (*(p) = (OS_U32) __get_BASEPRI())
  #if (OS_PRESERVE_PRIMASK == 1)
    #define OS_INT_Restore(p)   { OS_U32 primask;                            \
                                  primask = (OS_U32) __get_PRIMASK();        \
                                  __disable_interrupt();                     \
                                  __set_BASEPRI(*(p));                       \
                                  __set_PRIMASK(primask);                    \
  ...
  #else
    #define OS_INT_Restore(p)   { __disable_interrupt();                     \
                                  __set_BASEPRI(*(p));                       \
                                  __enable_interrupt();                      \
                                  __no_operation();                          \
                                  __no_operation();                          \
...
*/
  ...
  void OS_InitSysLocks         (void)          OS_TEXT_SECTION_ATTRIBUTE(OS_InitSysLocks);
  ...
  void OS__iar_system_Mtxinit  (__iar_Rmtx *m) OS_TEXT_SECTION_ATTRIBUTE(OS__iar_system_Mtxinit);
  void OS__iar_system_Mtxdst   (__iar_Rmtx *m) OS_TEXT_SECTION_ATTRIBUTE(OS__iar_system_Mtxdst);
  void OS__iar_system_Mtxlock  (__iar_Rmtx *m) OS_TEXT_SECTION_ATTRIBUTE(OS__iar_system_Mtxlock);
  void OS__iar_system_Mtxunlock(__iar_Rmtx *m) OS_TEXT_SECTION_ATTRIBUTE(OS__iar_system_Mtxunlock);
...
#if (defined(__clang__)  && (defined(__ARM_ARCH_8M_BASE__) || defined(__ARM_ARCH_8M_MAIN__))) \
 || (defined(__GNUC__)   && (defined(__ARM_ARCH_8M_BASE__) || defined(__ARM_ARCH_8M_MAIN__))) \
 || (defined(__ICCARM__) && defined(__ARM8M_BASELINE__) && (__CORE__ == __ARM8M_BASELINE__))  \
 || (defined(__ICCARM__) && defined(__ARM8M_MAINLINE__) && (__CORE__ == __ARM8M_MAINLINE__))
#include "arm_cmse.h"  // CMSIS header for ARMv8-M Security Extension
#ifndef   __ARM_FEATURE_CMSE
  #define __ARM_FEATURE_CMSE  0
#endif
...
typedef void (CMSE_NS_CALL *OS_NS_FUNC)(void);

CMSE_NS_ENTRY void          OS_SetMSP_ns   (unsigned long MSP_ns);
CMSE_NS_ENTRY void          OS_SetPSP_s    (unsigned long PSP_s);
CMSE_NS_ENTRY void          OS_SetPSPLIM_s (unsigned long PSPLIM_s);
CMSE_NS_ENTRY void          OS_SetCONTROL_s(unsigned long CONTROL_s);
CMSE_NS_ENTRY unsigned long OS_GetPSP_s    (void);
CMSE_NS_ENTRY unsigned long OS_GetPSPLIM_s (void);
CMSE_NS_ENTRY unsigned long OS_GetCONTROL_s(void);

void OS_TASK_SetContextExtensionARMTrustZone(void* pStack, unsigned long StackSize) OS_TEXT_SECTION_ATTRIBUTE(OS_TASK_SetContextExtensionARMTrustZone);
#endif  // ARMv8-M
