...
static QState Sporadic2_busy_e(Sporadic2 * const me);
static QMState const Sporadic2_busy_s = {
    &Sporadic2_active_s, // superstate
    Q_STATE_CAST(&Sporadic2_busy),
    Q_ACTION_CAST(&Sporadic2_busy_e),
    Q_ACTION_NULL, // no exit action
    Q_ACTION_NULL  // no initial tran.
};
    ...
    switch (e->sig) {
        ...
        case SPORADIC_B_SIG: {
            static struct {
                ...
                &Sporadic2_busy_s, // target state
                {
                    Q_ACTION_CAST(&Sporadic2_busy_e), // entry
                    Q_ACTION_NULL // zero terminator
                }
            };
            BSP_d5on();
...
static QState Sporadic2_busy_e(Sporadic2 * const me) {
    ...
    BSP_d5off();
    return QM_ENTRY(&Sporadic2_busy_s);
}
//${AOs::Sporadic2::SM::active::busy}
static QState Sporadic2_busy(Sporadic2 * const me, QEvt const * const e) {
    QState status_;
