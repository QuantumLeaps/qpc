...
static QState Sporadic3_busy_e(Sporadic3 * const me);
static QMState const Sporadic3_busy_s = {
    &Sporadic3_active_s, // superstate
    Q_STATE_CAST(&Sporadic3_busy),
    Q_ACTION_CAST(&Sporadic3_busy_e),
    Q_ACTION_NULL, // no exit action
    Q_ACTION_NULL  // no initial tran.
};
    ...
    switch (e->sig) {
        ...
        case SPORADIC_A_SIG: {
            static struct {
                ...
                &Sporadic3_busy_s, // target state
                {
                    Q_ACTION_CAST(&Sporadic3_busy_e), // entry
                    Q_ACTION_NULL // zero terminator
                }
            };
            BSP_d4on();
...
static QState Sporadic3_busy_e(Sporadic3 * const me) {
    ...
    BSP_d4off();
    return QM_ENTRY(&Sporadic3_busy_s);
}
//${AOs::Sporadic3::SM::active::busy}
static QState Sporadic3_busy(Sporadic3 * const me, QEvt const * const e) {
    QState status_;
