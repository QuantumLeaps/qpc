/*****************************************************************************
* Product: Board Support Package for AVR Butterfly
...
/*..........................................................................*/
void QF_onIdle(QF_INT_KEY_TYPE key) {      /* interrupts LOCKED, see NOTE01 */
    ...
    if ((UCSRA & (1 << UDRE)) != 0) {
        ...
        if (b != QS_EOD) {
            UDR = (uint8_t)b;               /* stick the byte to the TX UDR */
        }
    }
    else {
        QF_INT_UNLOCK(key);                            /* unlock interrupts */
...
uint8_t QS_onStartup(void const *arg) {
    ...
    UCSRB = (0 << RXEN) | (1 << TXEN) | (0 << RXCIE) | (0 << UDRIE);

    /* Async. mode, 8N1 */
    UCSRC = (0 << UMSEL) | (0 << UPM0) | (0 << USBS)
             | (3 << UCSZ0) | (0 << UCPOL);
...
void QS_onFlush(void) {
    ...
    while ((b = QS_getByte()) != QS_EOD) { /* next QS trace byte available? */
        while ((UCSRA & (1 << UDRE)) == 0) {           /* TX UDR not empty? */
        }
        UDR = (uint8_t)b;                   /* stick the byte to the TX UDR */
    }
}
