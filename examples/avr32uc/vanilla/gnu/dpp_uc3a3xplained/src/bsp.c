/*****************************************************************************
* Product: Board Support Package for UC3-A3-Xplained board, Vanilla kernel
...
void QF_onIdle() {           /* called with interrupts DISABLED, see NOTE01 */
    ...
    LED_On (LED0);           /* toggle the LED0 on and then off, see NOTE02 */
    LED_Off(LED0);
...
#elif defined NDEBUG
    /* go to sleep and re-enable interrupts at the same time, see NOTE03 */
...
* at the same time. This re-enables interrupts *atomically* as the machine
* goes to sleep.
*/
