/*****************************************************************************
* Product: DPP example, H8/SKP36077 board
...
/* Local objects -----------------------------------------------------------*/
    ...
    static uint32_t l_tickTime;                /* time at system clock tick */
...
void QF_onIdle(QF_INT_KEY_TYPE key) {
#ifdef Q_SPY
    if (SCI3.SSR.BIT.TDRE != 0) {                            /* TDRE ready? */
    ...
    set_imask_ccr(0); /* enable interrupts. The following SLEEP instruction */
    sleep();  /* will execute before entering any pending interrupt, NOTE01 */
#else
...
void QS_onFlush(void) {
    ...
    while ((b = QS_getByte()) != QS_EOD) { /* next QS trace byte available? */
        while (SCI3.SSR.BIT.TDRE == 0) {                 /* TDRE not ready? */
        }
