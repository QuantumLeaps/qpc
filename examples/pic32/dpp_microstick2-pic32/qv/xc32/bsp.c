...
//............................................................................
void QV_onIdle(void) { // CAUTION: called with interrupts DISABLED, see NOTE1

    // NOTE: not enough LEDs on the Microstick II board to implement
    // the idle loop activity indicator ...
    //LED_ON (); // blink the IDLE LED, see NOTE1
    //LED_OFF();

#ifdef Q_SPY
    QF_INT_ENABLE(); // enable interrupts, see NOTE1
...
#elif defined NDEBUG
    INTCONbits.TPC = 7U; // enable the Proximity Timer for all IPLs, NOTE1
    IPTMR = 4U;          // set the proximity timer to 4 CPU clocks
    QF_INT_ENABLE();     // enable CPU interrupts
    _wait();             // execute the WAIT instruction to stop the CPU
    INTCONbits.TPC = 0U; // disable the Proximity Timer for all IPLs
#else
    QF_INT_ENABLE(); // enable interrupts, see NOTE1
#endif
}
