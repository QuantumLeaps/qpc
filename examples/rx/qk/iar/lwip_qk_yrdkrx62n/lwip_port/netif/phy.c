...
static void  _phy_reg_set( uint16_t reg_addr, int32_t option );
static void  _phy_reg_read( uint16_t *data );
static void  _phy_reg_write( uint16_t data );
static void  _phy_ta_z0( void );
static void  _phy_ta_10( void );
static void  _phy_mii_write_1( void );
static void  _phy_mii_write_0( void );
static void  _phy_delay(uint32_t ctr);
...
int16_t phy_set_autonegotiate(void) {
    ...
    if (count == 0U) {
        count = PHY_AUTO_NEGOTIATON_WAIT;
        _phy_write(AN_ADVERTISEMENT_REG, 0x01E1U);
        _phy_write(BASIC_MODE_CONTROL_REG, 0x1200U);
    }
    --count;
    ...
    if ((reg & 0x20U) == 0U) {
        ...
        _phy_write(0x13U, 0x0006U);
        reg = _phy_read(0x14U);
        _phy_write(0x14U, (reg & 0x7FFFU));
        _phy_write(0x13U, 0x0000U);
