/*****************************************************************************
* Product: BSP for YRDKRX62N board, QK kernel, Renesas RX Standard Toolchain
...
#include "iodefine.h"
#include "Glyph.h"                                  /* for the graphics LCD */
#include "yrdkrx62n.h"
...
void BSP_init(void) {
    ...
    PORTD.DDR.BYTE  = 0xFFU;
    PORTE.DDR.BYTE |= 0x0FU;
    PORTD.DR.BYTE   = 0xFFU;
    PORTE.DR.BYTE  |= 0x0FU;             /* initialize all LEDs to OFF state */

    /* initialize the graphic LCD */
    Q_ALLEGE(GlyphOpen(&l_lcd, 0) == GLYPH_ERROR_NONE);
    GlyphNormalScreen(l_lcd);
    /*GlyphSetContrast(l_lcd, 10000U);*/
    GlyphClearScreen(l_lcd);
    GlyphSetFont(l_lcd, GLYPH_FONT_HELVR10);

    GlyphSetXY(l_lcd, 8U, 0U);
    GlyphString(l_lcd, (uint8_t *)"lwIP-QP example", 15U);

    GlyphSetXY(l_lcd, 2U, 16U);
    GlyphString(l_lcd, (uint8_t *)"IP=DHCP...", 10U);

    GlyphSetXY(l_lcd, 4U, 50U);
    GlyphString(l_lcd, (uint8_t *)"state-machine.com", 17U);
