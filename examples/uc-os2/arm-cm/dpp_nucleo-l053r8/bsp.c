...
void BSP_start(void) {
    ...
    static OS_STK philoStack[N_PHILO][72]; // stacks for the Philos
    for (uint8_t n = 0U; n < N_PHILO; ++n) {
        Philo_ctor(n);
        QACTIVE_START(AO_Philo[n],
            n + 1U,                  // QF-prio. of the AO
...
void BSP_displayPhilStat(uint8_t n, char const *stat) {
    ...
    if (stat[0] == 'e') {
        GPIOA->BSRR = (1U << B1_PIN); // turn LED on
    }
    else {
        GPIOA->BSRR = (1U << (B1_PIN + 16U)); // turn LED off
    }
...
void BSP_displayPaused(uint8_t const paused) {
    ...
    if (paused != 0U) {
        ...
        //GPIOA->BSRR = (LED_LD2 << 16U); // turn LED[n] off
    }
...
uint8_t QS_onStartup(void const *arg) {
    ...
    static uint8_t qsBuf[800]; // buffer for Quantum Spy
