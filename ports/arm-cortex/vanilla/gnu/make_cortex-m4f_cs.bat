@echo off
:: ===========================================================================
:: Product: QP/C buld script for ARM Cortex-M4F, Vanilla port, Code Sourcery
...
if "%1"=="" (
    ...
    set CCFLAGS=-g -c -mcpu=%ARM_CORE% -mfpu=%ARM_FPU% -mthumb -Wall -O
    set ASMFLAGS=-g -mcpu=%ARM_CORE% -defsym=FPU_VFP_V4_SP_D16=1
)
if "%1"=="rel" (
    echo rel selected
    set BINDIR=%QP_PRTDIR%\rel
    set CCFLAGS=-c -mcpu=%ARM_CORE% -mfpu=%ARM_FPU% -mthumb -Wall -Os -DNDEBUG
    set ASMFLAGS=-mcpu=%ARM_CORE% -defsym=FPU_VFP_V4_SP_D16=1
)
if "%1"=="spy" (
    echo spy selected
    set BINDIR=%QP_PRTDIR%\spy
    set CCFLAGS=-g -c -mcpu=%ARM_CORE% -mfpu=%ARM_FPU% -mthumb -Wall -O -DQ_SPY
    set ASMFLAGS=-g -mcpu=%ARM_CORE% -defsym=FPU_VFP_V4_SP_D16=1
)
