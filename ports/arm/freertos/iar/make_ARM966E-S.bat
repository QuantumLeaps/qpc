@echo off
:: ==========================================================================
:: Product: QP/C buld script for ARM, FreeRTOS.org port, IAR compiler
...
:: the IAR toolset...
::
set IAR_ARM=c:\tools\IAR\ARM_KS_6.40
...
:: relative path to the FreeRTOS directory. Please adjust, if necessary... 
set RTOS_INCDIR=..\..\..\..\..\FreeRTOS\Source\include

set ARM_CORE=ARM966E-S

if "%1"=="" (
    echo default selected
    set BINDIR=dbg
    set CCFLAGS=-I%IAR_ARM%\arm\inc -D STR91X_IAR -D DEBUG --debug --no_cse --no_unroll --no_inline --no_code_motion --no_tbaa --no_clustering --no_scheduling --endian little --cpu %ARM_CORE% -e --fpu None --dlib_config %IAR_ARM%\ARM\INC\c\DLib_Config_Normal.h --diag_suppress=Pa050 --interwork -On
    set ASMFLAGS=-s+ -w+ -r --cpu %ARM_CORE% --fpu None -I%IAR_ARM%\arm\inc
)
if "%1"=="rel" (
    echo rel selected
    set BINDIR=rel
    set CCFLAGS=-I%IAR_ARM%\arm\inc -D STR91X_IAR -D N_DEBUG --endian little --cpu %ARM_CORE% -e --fpu None --dlib_config %IAR_ARM%\ARM\INC\c\DLib_Config_Normal.h --diag_suppress=Pa050 --interwork -Ohz
    set ASMFLAGS=-s+ -w+ -r --cpu %ARM_CORE% --fpu None -I%IAR_ARM%\arm\inc
)
if "%1"=="spy" (
    echo spy selected
    set BINDIR=spy
    set CCFLAGS=-I%IAR_ARM%\arm\inc -D STR91X_IAR -D Q_SPY --debug --no_cse --no_unroll --no_inline --no_code_motion --no_tbaa --no_clustering --no_scheduling --endian little --cpu %ARM_CORE% -e --fpu None --dlib_config %IAR_ARM%\ARM\INC\c\DLib_Config_Normal.h --diag_suppress=Pa050 --interwork -On
    set ASMFLAGS=-s+ -w+ -r --cpu %ARM_CORE% --fpu None -I%IAR_ARM%\arm\inc
)
...
%CC% --cpu_mode arm   %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qa_sub.c
...
%CC% --cpu_mode arm   %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qeq_init.c
...
%CC% --cpu_mode arm   %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qf_tick.c
...
@echo on
%CC% --cpu_mode thumb %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qs.c
%CC% --cpu_mode arm   %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qs_.c
%CC% --cpu_mode arm   %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qs_blk.c
%CC% --cpu_mode arm   %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qs_byte.c
...
%CC% --cpu_mode arm   %CCFLAGS% %CCINC% -o%BINDIR% %SRCDIR%\qs_str.c
