@echo off
rem ==========================================================================
rem Product: QP/C library buld script for R8C, QK-nano port with NC30
...
if "%1"=="" (
    ...
    set CCFLAGS=-c -g -Wall -silent -%TARGET_MCU%
)
if "%1"=="rel" (
    echo rel selected
    set BINDIR=%QP_PRTDIR%\rel
    set CCFLAGS=-c -O3 -Wall -silent -%TARGET_MCU% -DNDEBUG
)
if "%1"=="spy" (
    echo spy selected
    set BINDIR=%QP_PRTDIR%\spy
    set CCFLAGS=-c -g -Wall -silent -%TARGET_MCU% -DQ_SPY
)
