@echo off
rem ==========================================================================
rem Product: QP/C library buld script for R8C Vanilla port with NC30
rem Last Updated for Version: 4.0.01
rem Date of the Last Update:  Aug 17, 2008
rem
...
rem the Renesas NC30 toolset...
rem
set RENESAS_M16C_DIR=C:\tools\Renesas\Hew\Tools\Renesas\nc30wa\v544r00
...
set PATH=%RENESAS_M16C_DIR%\bin;%PATH%
set LIB30=%RENESAS_M16C_DIR%\lib30
set CC=nc30.exe
set ASM=as30.exe
set LIB=lb30.exe
...
if "%1"=="" (
    ...
    set CCFLAGS=-c -g -Wall -%TARGET_MCU%
)
if "%1"=="rel" (
    echo rel selected
    set BINDIR=%QP_PRTDIR%\rel
    set CCFLAGS=-c -O3 -Wall -%TARGET_MCU% -DNDEBUG
)
if "%1"=="spy" (
    echo spy selected
    set BINDIR=%QP_PRTDIR%\spy
    set CCFLAGS=-c -g -Wall -%TARGET_MCU% -DQ_SPY
)
