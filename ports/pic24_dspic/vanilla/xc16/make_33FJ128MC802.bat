@echo off
:: ==========================================================================
:: Product: QP/C buld script for dsPIC33FJ128MC802, Vanilla port, MPLABX-XC16
...
:: the MPLABX-XC16 toolset...
::
set MPLABX_XC16="C:\tools\Microchip\xc16\v1.10"

:: adjust the following symbol to the PIC24/dsPIC device you're using
::
set PIC_DEVICE=33FJ128MC802

:: Typically, you don't need to modify this file past this line -------------

set CC=%MPLABX_XC16%\bin\xc16-gcc.exe
set ASM=%MPLABX_XC16%\bin\xc16-as.exe
set LIB=%MPLABX_XC16%\bin\xc16-ar.exe
...
if "%1"=="" (
    ...
    set BINDIR=dbg
    set CCFLAGS=-mcpu=%PIC_DEVICE% -MMD -MF c -g -omf=elf -msmart-io=1 -msfr-warn=off -Wall -O0
)
if "%1"=="rel" (
    echo rel selected
    set BINDIR=rel
    set CCFLAGS=-mcpu=%PIC_DEVICE% -MMD -MF c -omf=elf -msmart-io=1 -msfr-warn=off -Wall -Os -DNDEBUG
)
if "%1"=="spy" (
    echo spy selected
    set BINDIR=spy
    set CCFLAGS=-mcpu=%PIC_DEVICE% -MMD -MF c -g -omf=elf -msmart-io=1 -msfr-warn=off -Wall -O0 -DQ_SPY
)
