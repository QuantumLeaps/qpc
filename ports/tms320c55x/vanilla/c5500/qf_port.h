/*****************************************************************************
* Product:  QF/C, TMS320C55x, vanilla port, TI-C5500 compiler
...
*
* NOTE04:
* TMS320C55x automatically disables interrupts upon the entry to an ISR
...
* that you can call QF functions), you need to clear the INTM mask. To avoid
* inadvertent interrupt nesting, every ISR needs to call QF_ISR_ENTRY()
* right after the entry and QF_ISR_EXIT() right before the exit. The macro
* QF_ISR_ENTRY() saves and clears the interrupt enable masks (IER0 and IER1).
* After clearing these masks interrupts can be safely re-enabled. The macro
* QF_ISR_EXIT() disables interrupts and restores the interrupt enable masks.
*/
