...
QSchedStatus QXK_schedLock(uint_fast8_t const ceiling) {
    ...
    if (ceiling > QXK_priv_.lockCeil) {
        QS_BEGIN_PRE(QS_SCHED_LOCK, 0U)
            ...
            QS_2U8_PRE((uint8_t)QXK_priv_.lockCeil, (uint8_t)ceiling);
        QS_END_PRE()
...
void QXK_schedUnlock(QSchedStatus const stat) {
    ...
    if (stat != 0xFFU) {
        ...
        QS_BEGIN_PRE(QS_SCHED_UNLOCK, 0U)
            ...
            QS_2U8_PRE((uint8_t)QXK_priv_.lockCeil, (uint8_t)prevCeil);
        QS_END_PRE()
...
void QXK_contextSw_(QActive * const next) {
    ...
    if (next != (QActive *)0) { // next is NOT idle?
        QS_BEGIN_PRE(QS_SCHED_NEXT, next->prio)
            QS_TIME_PRE(); // timestamp
            QS_2U8_PRE(next->prio, prev_prio);
        QS_END_PRE()
    }
    else { // going to idle
        QS_BEGIN_PRE(QS_SCHED_IDLE, prev_prio)
            QS_TIME_PRE(); // timestamp
            QS_U8_PRE(prev_prio);
        QS_END_PRE()
    }
...
int_t QF_run(void) {
    ...
    QXK_priv_.prev = (QActive *)0;

    // officially switch to the idle context
    QXK_contextSw_(QXK_priv_.next);

    // activate AOs/threads to process events posted so far
    if (QXK_sched_() != 0U) {
        QXK_activate_();
    }
