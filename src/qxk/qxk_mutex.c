    ...
    QTimeEvtCtr const nTicks)
{
    ...
    if (me->ao.eQueue.nFree == 0U) {
        ...
        QS_BEGIN_PRE(QS_MTX_LOCK, curr->super.prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this mutex
            QS_U8_PRE((uint8_t)me->ao.eQueue.head); // holder prio
            QS_U8_PRE((uint8_t)me->ao.eQueue.nFree); // nesting
        QS_END_PRE()
    ...
    // is the mutex locked by this thread already (nested locking)?
        ...
        QS_BEGIN_PRE(QS_MTX_BLOCK, curr->super.prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this mutex
            QS_2U8_PRE((uint8_t)me->ao.eQueue.head, // holder prio
...
bool QXMutex_tryLock(QXMutex * const me) {
    ...
    if (me->ao.eQueue.nFree == 0U) {
        ...
        QS_BEGIN_PRE(QS_MTX_LOCK, curr->prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this mutex
            QS_U8_PRE((uint8_t)me->ao.eQueue.head); // holder prio
            QS_U8_PRE((uint8_t)me->ao.eQueue.nFree); // nesting
    ...
    // is the mutex locked by this thread already (nested locking)?
        ...
        QS_BEGIN_PRE(QS_MTX_BLOCK_ATTEMPT, curr->prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this mutex
            QS_2U8_PRE((uint8_t)me->ao.eQueue.head, // holder prio
...
void QXMutex_unlock(QXMutex * const me) {
    ...
    if (me->ao.eQueue.nFree == 1U) {
        ...
        QS_BEGIN_PRE(QS_MTX_UNLOCK, curr->prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this mutex
            QS_2U8_PRE((uint8_t)me->ao.eQueue.head, // holder prio
                        0U); // nesting
        ...
        if (QPSet_notEmpty(&me->waitSet)) {
            ...
            QS_BEGIN_PRE(QS_MTX_LOCK, thr->super.prio)
                QS_TIME_PRE();    // timestamp
                QS_OBJ_PRE(me);   // this mutex
                QS_U8_PRE((uint8_t)me->ao.eQueue.head); // holder prio
                QS_U8_PRE((uint8_t)me->ao.eQueue.nFree); // nesting
        ...
        QS_BEGIN_PRE(QS_MTX_UNLOCK_ATTEMPT, curr->prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this mutex
            QS_U8_PRE((uint8_t)me->ao.eQueue.head); // holder prio
            QS_U8_PRE((uint8_t)me->ao.eQueue.nFree); // nesting
