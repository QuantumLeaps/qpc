    ...
    QTimeEvtCtr const nTicks)
{
    ...
    if (me->count > 0U) {
        ...
        QS_BEGIN_PRE(QS_SEM_TAKE, curr->super.prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this semaphore
            QS_2U8_PRE(curr->super.prio, me->count);
        QS_END_PRE()
    }
        ...
        QS_BEGIN_PRE(QS_SEM_BLOCK, curr->super.prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this semaphore
            QS_2U8_PRE(curr->super.prio, me->count);
        QS_END_PRE()
...
bool QXSemaphore_tryWait(QXSemaphore * const me) {
    ...
    if (me->count > 0U) {
        ...
        QS_BEGIN_PRE(QS_SEM_TAKE, curr->prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this semaphore
            QS_2U8_PRE(curr->prio, me->count);
        QS_END_PRE()
    }
    else { // the semaphore is NOT available (would block)
        taken = false;

        QS_BEGIN_PRE(QS_SEM_BLOCK_ATTEMPT, curr->prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this semaphore
            QS_2U8_PRE(curr->prio, me->count);
        QS_END_PRE()
...
bool QXSemaphore_signal(QXSemaphore * const me) {
    ...
    if (QPSet_notEmpty(&me->waitSet)) {
        ...
        QS_BEGIN_PRE(QS_SEM_TAKE, thr->super.prio)
            QS_TIME_PRE();    // timestamp
            QS_OBJ_PRE(me);   // this semaphore
            QS_2U8_PRE(thr->super.prio, me->count);
        QS_END_PRE()
