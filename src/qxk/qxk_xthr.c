...
QEvt const * QXThread_queueGet(QTimeEvtCtr const nTicks) {
    ...
    if (thr->super.eQueue.frontEvt != (QEvt *)0) {
    ...
    #ifndef Q_UNSAFE
        thr->super.eQueue.nFree_dis = (QEQueueCtr)~nFree;
    #endif // ndef Q_UNSAFE
        ...
        if (nFree <= thr->super.eQueue.end) {
            ...
            QS_BEGIN_PRE(QS_QF_ACTIVE_GET, thr->super.prio)
                QS_TIME_PRE();      // timestamp
                QS_SIG_PRE(e->sig); // the signal of this event
                QS_OBJ_PRE(&thr->super); // this active object
            ...
            QS_BEGIN_PRE(QS_QF_ACTIVE_GET_LAST, thr->super.prio)
                QS_TIME_PRE();      // timestamp
                QS_SIG_PRE(e->sig); // the signal of this event
                QS_OBJ_PRE(&thr->super); // this active object
                QS_2U8_PRE(QEvt_getPoolNum_(e), e->refCtr_);
    ...
    QTimeEvtCtr const nTicks)
{
    ...
    if (nTicks != QXTHREAD_NO_TIMEOUT) {
    ...
    #ifndef Q_UNSAFE
        me->timeEvt.ctr_dis = (QTimeEvtCtr)~nTicks;
    #endif // ndef Q_UNSAFE
        ...
        if ((me->timeEvt.flags & QTE_FLAG_IS_LINKED) == 0U) {
            me->timeEvt.flags |= QTE_FLAG_IS_LINKED; // mark as linked

            uint8_t const tickRate = me->timeEvt.tickRate;
    ...
    #ifndef Q_UNSAFE
            me->timeEvt.next_dis = (uintptr_t)~Q_PTR2UINT_CAST_(me->timeEvt.next);
            QTimeEvt_timeEvtHead_dis_[tickRate] =
                (uintptr_t)~Q_PTR2UINT_CAST_(&me->timeEvt);
    #endif // ndef Q_UNSAFE
        }
    }
}
...
bool QXThread_teDisarm_(QXThread * const me) {
    ...
    #ifndef Q_UNSAFE
        me->timeEvt.ctr_dis = (QTimeEvtCtr)~0U;
    #endif // ndef Q_UNSAFE
    }
