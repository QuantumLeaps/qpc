...
if "%1"=="" (
    ...
    set CCFLAGS=-g -c --cpu %ARM_CORE% --apcs=interwork --split_sections -O0
    set ASMFLAGS=-g --cpu %ARM_CORE% --apcs=interwork
)
if "%1"=="rel" (
    echo rel selected
    set BINDIR=%QP_PRTDIR%\rel
    set CCFLAGS=-c --cpu %ARM_CORE% --apcs=interwork --split_sections -O3 -Otime -DNDEBUG
    set ASMFLAGS=--cpu %ARM_CORE% --apcs=interwork
)
if "%1"=="spy" (
    echo spy selected
    set BINDIR=%QP_PRTDIR%\spy
    set CCFLAGS=-g -c --cpu %ARM_CORE% --apcs=interwork --split_sections -O0 -DQ_SPY
    set ASMFLAGS=-g --cpu %ARM_CORE% --apcs=interwork
)
